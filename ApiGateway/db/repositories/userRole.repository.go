...
func (this *UserRoleRepository) GetUserRoles(userId int64) ([]*models.Role, error) {
	query := `
...
func (this *UserRoleRepository) AssignRoleToUser(userId int64, roleId int64) error {
	query := "INSERT INTO user_roles (user_id, role_id) VALUES (?, ?)"
	_, err := this.db.Exec(query, userId, roleId)
	if err != nil {
		return err
	}
	return nil
...
func (this *UserRoleRepository) RemoveRoleFromUser(userId int64, roleId int64) error {
...
func (this *UserRoleRepository) GetUserPermissions(userId int64) ([]*models.Permission, error) {
	query := `
...
func (this *UserRoleRepository) HasPermission(userId int64, permissionName string) (bool, error) {
	query := `
		SELECT COUNT(*) > 0
		FROM user_roles ur
		INNER JOIN role_permissions rp ON ur.role_id = rp.role_id
	...
	var exists bool
	err := this.db.QueryRow(query, userId, permissionName).Scan(&exists)
	if err != nil {
		return false, err
	}
	return exists, nil
...
func (this *UserRoleRepository) HasRole(userId int64, roleName string) (bool, error) {
	query := `
		SELECT COUNT(*) > 0
		FROM user_roles ur
		INNER JOIN roles r ON ur.role_id = r.id
	...
	var exists bool
	err := this.db.QueryRow(query, userId, roleName).Scan(&exists)
	if err != nil {
		return false, err
	}
	return exists, nil
...
func (this *UserRoleRepository) HasAllRoles(userId int64, roleNames []string) (bool, error) {
	...
	row := this.db.QueryRow(query, len(roleNames), userId, roleNamesStr)
...
func (this *UserRoleRepository) HasAnyRole(userId int64, roleNames []string) (bool, error) {
